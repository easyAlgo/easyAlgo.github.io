...
], function(app, Parser, fs)
{
	...
	var terminalReader = {
		...
		read : function (validator, action) {
			var _this = this;
			// do apply for write non writing text
			_this.$scope.$apply();
    ...
    app.controller('easyAlgoEditor', function($scope, $modal, $timeout, $injector, templatePreloader, popUpManager){
        ...
        $scope.runAlgo = function(){
			...
			// run algo
            var runner = require('easyAlgoRunner');
            runner.run(algo, terminalWritter, terminalReader, function(){				
				terminalWritter.info('Fin de l\'algorithme');
			});
        };
		...
		$scope.initAlgos = function() {
			fs.listDir('algos', function(dirContent) {
				...
				for (var i in dirContent) {
					(function(index, entryFile) {
						fs.readFileEntry(entryFile, function(content){
							try {
								var tab = {};
								if (JSON && JSON.parse) {
								...
								tab.renameMode = false;
								tab.active = true;
								tab.filePath = entryFile.fullPath.slice(1);
