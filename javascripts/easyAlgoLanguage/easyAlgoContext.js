define(['easyAlgoRuntimeException', 'easyAlgoConfiguration'], function(RuntimeException, easyAlgoConfiguration){
	 ...
	 * @param parent the parent context (facultative)
	 * @param contextName the context name (for exemple if i's a function context, the name must be the function name)
	 */
	var Context = function(parent, contextName) {
		if (parent) {
	...
	Context.prototype = {
		...
		getAccessibleFunctions : function(){
			...
			if (this.getParent()) {
				var parentFunction = this.getParent().getAccessibleFunctions();
				for (var i in parentFunction) {
					ret[i] = parentFunction[i];
				}
			}
			
			for  (functionName in this.functions) {
				ret[functionName] = this.functions[i];
			}
